...
- 🐳 **Contenedores Docker**: Gestión completa de Docker sin usar la CLI: lista, arranca, detén, reinicia y elimina contenedores, y consulta sus **logs** en vivo. Despliega aplicaciones desde repositorios privados o públicos de GitHub con **re-despliegue automático en 1 clic** ("Actualizar desde Git") que guarda la configuración y el **token cifrado en reposo (AES-256-GCM)** en SQLite, descargando los últimos cambios y reconstruyendo el contenedor sin pedir credenciales. Permite crear contenedores desde imágenes del registro o compilando un Dockerfile/plantillas integradas (Node, Python, PHP, Estático), con mapeo de puertos, variables de entorno, **volúmenes persistentes** y proxy Nginx + SSL opcional por dominio. Incluye editor de **Dockerfile** y de **docker-compose** con despliegue en un clic.
