'use strict';
// ============================================================
//  TecXPaneL — lib/backups/remote.js — ejecutor de rclone para backups remotos
//  Copia literal de lib/backupRemote.js v1, con imports reorganizados.
// ============================================================
...
async function uploadArchive({ filename }) {
  if (!B.manifest.isValidBackupFilename(filename)) return { ok: false, message: 'Nombre de backup inválido' };
  const local = path.join(B.manifest.BACKUP_DIR, filename);
...
async function downloadArchive({ filename }) {
  ...
  const { env, cleanup, remote } = await buildEnv();
  try {
    await runRclone(R.copyArgs(joinRemote(remote, filename), B.manifest.BACKUP_DIR), env);
    return { ok: true };
  } catch (e) {
    return { ok: false, message: (e.stderr || e.message || '').toString().slice(0, 300) };
  } finally { await cleanup(); }
