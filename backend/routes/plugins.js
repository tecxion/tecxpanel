...
const PLUGINS = {
  ...
  certbot: {
    label: 'Certbot', desc: 'Certificados SSL Let\'s Encrypt',
...
router.get('/', wrap(async (req, res) => {
  ...
  for (const [key, p] of Object.entries(PLUGINS)) {
    ...
    result.push({ id: key, label: p.label, desc: p.desc, installed: r.ok });
  }
  ok(res, result);
}));
