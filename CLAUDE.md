...
- `js/<dominio>.js` (v1, módulos ES con `export {}` para back-compat) — `auth.js`, `dashboard.js`, `docker.js`, `databases.js`, `dns.js`, `files.js`, `firewall.js`, `logs.js`, `mail.js`, `n8n.js`, `notifications.js`, `palette.js`, `plugins.js`, `settings.js`, `ssl.js`, `terminal.js`, `websites.js`, `apps.js`, `catalog.js`, `backups.js`, `cron.js`. Las funciones se registran en `window` para que los handlers `onclick=` inline siguen funcionando.
